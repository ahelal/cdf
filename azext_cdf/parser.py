...
class ConfigParser:
    ...
    def _validate_conf(self, config_filepath):
        try:
            ...
            self.config = schema_obj.validate(self.config)
    ...
    def _extra_validate(self):
        ...
        for test in self.config.get(CONFIG_TESTS, {}):
            for hooks_ops in self.config[CONFIG_TESTS][test].get(CONFIG_EXPECT, {}).get(CONFIG_HOOKS, []):
                for hook in hooks_ops:
                    if hook not in hooks:
                        raise CLIError(f"unknown hook name '{hook}' in expect test '{test}'")
            upgrade_from_names = []
            for upgrade in self.config[CONFIG_TESTS][test].get("upgrade_from", []):
    ...
    def _setup_load_file_references(self):
        ...
        for test_name in self.config[CONFIG_TESTS].keys():
            if self.config[CONFIG_TESTS][test_name].get(CONFIG_FILE, False):  # load test from another dir
                self.config[CONFIG_TESTS][test_name][CONFIG_FILE] = self.interpolate(FIRST_PHASE, self.config[CONFIG_TESTS][test_name][CONFIG_FILE], f"test {test_name} key {CONFIG_FILE}")
                test_data = self._read_config(self.config[CONFIG_TESTS][test_name][CONFIG_FILE])
                self.config[CONFIG_TESTS][test_name] = {**self.config[CONFIG_TESTS][test_name], **test_data}
                self._validate_conf(self.config[CONFIG_TESTS][test_name][CONFIG_FILE])  # revalidate after loading data

    def _setup_test(self):
        if not self.test:
            return
        if not self.get_test(self.test):
            raise CLIError(f"Could not find test {self.test}")

        self.config[CONFIG_NAME] = self.config[CONFIG_TESTS][self.test].get(CONFIG_NAME, f"{self.config[CONFIG_NAME]}_{self.test}_test")
        self.config[CONFIG_TESTS][self.test][CONFIG_DESCRIPTION] = self.config[CONFIG_TESTS][self.test].get(CONFIG_DESCRIPTION, f"{self.config[CONFIG_NAME]} {self.test} test")
        self.config[CONFIG_LOCATION] = self.config[CONFIG_TESTS][self.test].get(CONFIG_LOCATION, self.config[CONFIG_LOCATION])
        self.config[CONFIG_RG_MANAGED] = self.config[CONFIG_TESTS][self.test].get(CONFIG_RG_MANAGED, self.config[CONFIG_RG_MANAGED])
        self.config[CONFIG_UP] = self.config[CONFIG_TESTS][self.test].get(CONFIG_UP, self.config[CONFIG_UP])
        self.config[CONFIG_PROVISIONER_BINARY_PATH] = self.config[CONFIG_TESTS][self.test].get(CONFIG_PROVISIONER_BINARY_PATH, self.config[CONFIG_PROVISIONER_BINARY_PATH])
        self.config[CONFIG_VARS] = {**self.config[CONFIG_VARS], **self.config[CONFIG_TESTS][self.test].get(CONFIG_VARS, {})}
        self.config[CONFIG_PARAMS] = {**self.config[CONFIG_PARAMS], **self.config[CONFIG_TESTS][self.test].get(CONFIG_PARAMS, {})}

        if self.config[CONFIG_TESTS][self.test].get(CONFIG_RG, None):
            self.config[CONFIG_RG] = self.config[CONFIG_TESTS][self.test][CONFIG_RG]
        elif self.config[CONFIG_RG_MANAGED]:
            self.config[CONFIG_RG] = f"{self.config[CONFIG_RG]}_{self.test}_test"
        # Don't do anything use cdf self.config[CONFIG_RG]
    ...
    def _setup_first_phase_interpolation(self, state_locking, remove_tmp=False):
        ...
        self.first_phase_vars[CONFIG_CDF][CONFIG_TMP] = self.interpolate(FIRST_PHASE, self.config[CONFIG_TMP], context=f"key {CONFIG_TMP}")
        ...
        self.config[CONFIG_STATE_FILEPATH] = self.interpolate(FIRST_PHASE, self.config[CONFIG_STATE_FILEPATH], context=f"key {CONFIG_STATE_FILEPATH}")
        self.state = State(self.config[CONFIG_STATE_FILEPATH], locking=state_locking)  # initialize state
        self.jinja_env.globals["store"] = self.state.store_get  # setup store functions in jinja2

        self.first_phase_vars[CONFIG_CDF][CONFIG_NAME] = self.interpolate(FIRST_PHASE, self.config[CONFIG_NAME], f"key {CONFIG_NAME}")
        if CONFIG_VARS in self.config:
            # lazy variable resolve
            for key, value in self.config[CONFIG_VARS].items():
                try:
        ...
        self.first_phase_vars[CONFIG_CDF][CONFIG_RG] = self.interpolate(FIRST_PHASE, self.config[CONFIG_RG], f"key {CONFIG_RG}")
        self.first_phase_vars[CONFIG_CDF][CONFIG_LOCATION] = self.interpolate(FIRST_PHASE, self.config[CONFIG_LOCATION], f"key {CONFIG_LOCATION}")
        self.config[CONFIG_UP] = self.interpolate(FIRST_PHASE, self.config[CONFIG_UP], f"key {CONFIG_UP}")
        self.config[CONFIG_PROVISIONER_BINARY_PATH] = self.interpolate(FIRST_PHASE, self.config[CONFIG_PROVISIONER_BINARY_PATH], f"key {CONFIG_PROVISIONER_BINARY_PATH}")
    ...
    def interpolate_delayed_variable(self):
        ...
        for k in self._delayed_vars:
            self.first_phase_vars[CONFIG_VARS][k] = self.interpolate(SECOND_PHASE, self.config[CONFIG_VARS][k], f"variables in config in delayed interpolate '{k}'")
    ...
    def get_hooks(self, format_list=True):
        ...
        if format_list:
            hooks = []
            for k, _ in self.config[CONFIG_HOOKS].items():
                hooks.append(k)
            return hooks
        return self.config[CONFIG_HOOKS].items()

    def get_test(self, test_name, expect=None, hook=None):
        ''' Return test dic '''

        test_obj = self.config.get(CONFIG_TESTS, {}).get(test_name, {})
    ...
    def test_hooks(self, test_name):
        ...
        hooks = []
        for k in self.config.get(CONFIG_TESTS, {}).get(test_name, {}).get(CONFIG_EXPECT, {}).get(CONFIG_HOOKS, []):
    ...
    def upgrade_flaten(self, test_name):
        ...
        upgrade_path = []
        upgrades = self.config[CONFIG_UPGRADE] + self.get_test(test_name).get(CONFIG_UPGRADE)
    ...
    def tests(self):
        ...
        tests = []
        for k, _ in self.config[CONFIG_TESTS].items():
            tests.append(k)
        return tests
    ...
    def managed_resource(self):
        ...
        return self.config[CONFIG_RG_MANAGED]
    ...
    def up_location(self):
        ...
        return self.config[CONFIG_UP]

    @property
    def provisioner(self):
        ''' returns CDF provisioner '''

        return self.config["provisioner"]

    @property
    def provisioner_path(self):
        ''' returns CDF provisioner path if any '''

        if self.provisioner == "terraform" and self.config[CONFIG_PROVISIONER_BINARY_PATH].strip() == "":
            return "terraform"
        if self.provisioner == "terraform" and self.config[CONFIG_PROVISIONER_BINARY_PATH].strip() != "":
            return self.config[CONFIG_PROVISIONER_BINARY_PATH].strip()

        if self.provisioner == "bicep" and self.config[CONFIG_PROVISIONER_BINARY_PATH].strip() == "":
            return "bicep"
        if self.provisioner == "bicep" and self.config[CONFIG_PROVISIONER_BINARY_PATH].strip() != "":
            return self.config[CONFIG_PROVISIONER_BINARY_PATH].strip()
    ...
    def deployment_mode(self):
        ...
        return self.config[CONFIG_DEPLOYMENT_COMPLETE]
